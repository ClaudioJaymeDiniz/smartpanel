...
import { View, Text as RNText, ActivityIndicator, ScrollView, RefreshControl, TouchableOpacity, StyleSheet } from 'react-native';
import { Ionicons } from '@expo/vector-icons';

import { THEME } from '@/styles/theme';
import Container from '@/components/common/Container';

import { useProjectDetails } from '@/presentation/projects/hooks/useProjectDetails'; // ajuste o caminho relativo
import { ProjectHero } from '@/presentation/projects/components/ProjectHero';
import { ProjectTabs } from '@/presentation/projects/components/ProjectTabs';
...
export default function ProjectDetails() {
  ...
  // Configuração do Header dinâmica baseada nos dados do Hook
  useEffect(() => {
    navigation.setOptions({
      title: project?.name || (initialName as string) || 'Projeto',
      headerTintColor: projectColor,
      ...
      headerRight: () => isOwner ? (
        <TouchableOpacity 
          onPress={() => router.push({ pathname: "/(project)/edit", params: { id: projectId } })} 
          style={{ marginRight: 10 }}
        >
          <Ionicons name="settings-outline" size={22} color={projectColor} />
        </TouchableOpacity>
  ...
  }, [projectId, initialName, isOwner, navigation, project?.name, projectColor, router]);
  ...
  return (
    <View style={styles.screenContainer}>
      <ScrollView 
        refreshControl={
          <RefreshControl refreshing={refreshing} onRefresh={handleRefresh} colors={[projectColor]} />
        }
      >
        <Container>
          <ProjectHero 
            name={project?.name || (initialName as string) || 'Projeto'}
          ...
          {activeSection === 'forms' ? (
            <ProjectFormList 
              ...
              projectId={projectId}
              isOwner={isOwner} 
              onFormPress={(formId) => router.push(`/(form)/${formId}`)} 
              onNewFormPress={isOwner ? () => router.push(`/(form)/new?projectId=${projectId}&color=${encodeURIComponent(projectColor)}`) : undefined}
            />
          ) : (
            <>
              <ProjectTeamList 
                ...
                onInvitePress={isOwner ? () => router.push({
                  pathname: '/(project)/invite',
                  params: { id: projectId, name: project?.name, color: projectColor }
                }) : undefined}
                isOwner={isOwner}
              />
